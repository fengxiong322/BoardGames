...
public class SnakesAndLadders extends BoardGames {
  ...
  private void moveOnce() {
    int x = getPieceX(currentPlayer);
    int y = getPieceY(currentPlayer);
    if (y % 2 == 0) {//If y is even
      if (x == 9){
        y++;
        setPiece(currentPlayer, x, y);// Move up the y
      }else{
        x++;
        setPiece(currentPlayer, x, y);// Move up the x
      }
    } else {
      if (x == 0 && y != 0){
        y++;
  ...
  public boolean move(int numOfMoves) {// returns true if the moving player has won
    setBoard(getPieceX(currentPlayer),getPieceY(currentPlayer), -1);
    for (int i = 0; i < numOfMoves; i++) {
      moveOnce();
      if (getPieceY(currentPlayer) == 9 && getPieceX(currentPlayer) == 0)// when at the top row and first column
        return true;// player has won
    }
    int currentSnake = checkSnake();
    if (currentSnake != -1) {
      setPiece(currentPlayer, snakes[currentSnake][2], snakes[currentSnake][3]);
    }
    int currentLadder = checkLadder();
    if (currentLadder != -1) {
      setPiece(currentPlayer, ladders[currentSnake][2], ladders[currentSnake][3]);
    }
    setBoard(getPieceX(currentPlayer),getPieceY(currentPlayer), currentPlayer);
  ...
  public int checkSnake() {// returns the snake number, or -1 if there is no such snake
    for (int i = 0; i < numSnakes; i++)
      if (snakes[i][0] == getPieceX(currentPlayer) && snakes[i][1] == getPieceY(currentPlayer))
        return i;
    return -1;
  }
  ...
  public int checkLadder() {// returns the ladder number, or -1 if there is no such ladder
    for (int i = 0; i < numLadders; i++)
      if (ladders[i][0] == getPieceX(currentPlayer) && ladders[i][1] == getPieceY(currentPlayer))
        return i;
    return -1;
  }
}